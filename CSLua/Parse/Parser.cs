...
public sealed class Parser
{
	...
	private void Constructor(ExpDesc t)
	{
		...
		// it is also not implemented here for now.
		// This does not affect the logic, only the efficiency.
		// var ins = fs.Proto.Code[pc];
		// ins.SETARG_B(0);
		// ins.SETARG_C(0);

		// Forget it, let’s implement it anyway. It’s easier to check if the generated bytecode matches luac.
		// Instruction is a value type. Sigh.
		fs.Proto.Code[pc] = fs.Proto.Code[pc]
			.SETARG_B(Integer2FloatingPointByte((uint)cc.NumArray))
			.SETARG_C(Integer2FloatingPointByte((uint)cc.NumRecord));
	}
